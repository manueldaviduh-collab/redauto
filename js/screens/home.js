  ...
  container.innerHTML = `
    <header class="top-header top-header--home">
      ...
      <div class="top-header__actions">
        <a href="#/notificaciones" class="icon-btn" id="btn-bell" aria-label="Notificaciones">
          ${icon('bell', { size: 21 })}<span class="dot-badge" aria-hidden="true" ${notificationService.unreadCount() ? '' : 'hidden'}></span>
        </a>
    ...
    <div class="screen-pad">
      ...
      <section class="section section--vehicles" aria-label="Mis vehículos">
        <div class="section__head">
          <h2 class="section__title">Mis vehículos</h2>
          <a href="#/mis-vehiculos" class="section__link">Gestionar</a>
        </div>
        ${garage.length ? `
          <div class="h-scroll vehicle-chip-row" id="garage-chip-row">
            ${garage.map((v) => vehicleChip(v, v.id === activeId)).join('')}
            <button type="button" class="vehicle-chip vehicle-chip--add" data-add-vehicle>
              ${icon('plusCircle', { size: 17 })}<span>Agregar</span>
            </button>
          </div>
        ` : `
          <button type="button" class="empty-vehicle-prompt" data-add-vehicle>
            <span class="empty-vehicle-prompt__icon">${icon('car', { size: 20 })}</span>
            <span class="empty-vehicle-prompt__text">
              <strong>Agrega tu vehículo</strong>
              <small>Así te mostramos qué repuestos son compatibles</small>
            </span>
            ${icon('chevronRight', { size: 16 })}
          </button>

          <section class="vehicle-picker" aria-label="Buscar por vehículo">
...
function bindVehiclePicker(container) {
  ...
  if (!brandEl) return; // sólo existe cuando el garage está vacío
...
function vehicleChip(vehicle, isActive) {
  return `
    <button type="button" class="vehicle-chip ${isActive ? 'is-active' : ''}" data-vehicle-id="${vehicle.id}">
      ${icon('car', { size: 16 })}
      <span>${escapeHtml(vehicle.brand)} ${escapeHtml(vehicle.model)} ${vehicle.year}</span>
    </button>`;
}

function bindGarage(container) {
  container.querySelectorAll('[data-add-vehicle]').forEach((btn) => {
    btn.addEventListener('click', () => navigate('/mis-vehiculos'));
  });
  container.querySelectorAll('[data-vehicle-id]').forEach((chip) => {
    chip.addEventListener('click', () => {
      const id = chip.dataset.vehicleId;
      vehicleService.setActive(id);
      const v = vehicleService.getGarage().find((veh) => veh.id === id);
      if (!v) return;
      navigate(`/buscar?brand=${encodeURIComponent(v.brand)}&model=${encodeURIComponent(v.model)}&year=${v.year}`);
    });
  });
}
